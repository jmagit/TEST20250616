...
async def python_error():
    try:
        res = await io_bound(r.get, 'http://localhost:8008/error')
        ui.notify(res.text)
    except Exception as e:
        apm.capture_exception()
        ui.notify(f'{e}')
...
async def js_error():
    try:
        res = await ui.run_javascript('fetch("http://localhost:8008/error")')
...
async def gen_custom_message(text_message):
    try:
        res = await io_bound(r.get, 'http://localhost:8008/custom_message/' + str(text_message))
        ui.notify(res.text)
    except Exception as e:
        apm.capture_exception()
        ui.notify(f'{e}')
